...
//WiFi postavke
const char* WIFI_SSID     = "Kabinet_39";
const char* WIFI_PASSWORD = "el-robot2018";
...
<h1>🔐 Kontrola Pristupa</h1>
  ...
  html += "<p>IP adresa: <strong>" + WiFi.localIP().toString() + "</strong></p>";
  html += "</div>";
...
void setup() {
  ...
  // WiFi
  oledShow("Spajanje na WiFi...", WIFI_SSID, "", "");
  Serial.printf("[INFO] Spajanje na WiFi: %s\n", WIFI_SSID);
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    oledShow("WiFi spojen!", WiFi.localIP().toString(), "", "");
    delay(2000);
  } else {
    Serial.println("\n[UPOZORENJE] WiFi nije spojen. Web sučelje nedostupno.");
    oledShow("WiFi NIJE spojen", "Offline mod", "", "");
    delay(2000);
  }
